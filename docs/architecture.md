...
Windows は PFX と公開証明書を照合し、拡張子ではなくヘッダーで検出した PE ファイルを署名します。
署名前後で対象が変わっていないことを確認し、署名者、fingerprint、publisher を検証します。
署名に使う一時ファイル、macOS の一時キーチェーン、Windows で一時追加した証明書は終了時に削除し、後始末の失敗もエラーとして扱います。

GitHub App トークンは取得と公開で別々に発行し、どちらも対象リポジトリ一つへ限定します。
署名・公開の承認待ちへ公開用トークンを持ち越さず、ジョブ終了時に失効させます。
...
ファイル名や個数の固定一覧ではなく、実際のファイルと更新メタデータの参照、サイズ、SHA-512、バージョンを照合します。
