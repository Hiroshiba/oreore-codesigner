...
6. 起動と主要機能を確認します。手動更新は対象 Release から新しい ZIP を取得し、展開したアプリの署名者と証明書を確認してから、アプリを終了して既存アプリを置き換えます。アプリ内更新は対象アプリの手順で行い、どちらも更新後の起動、バージョン、設定と利用者データの保持を[検証手順](verification.md)で確認します。
...
5. 起動と主要機能を確認します。手動更新は対象 Release から新しい通常 NSIS インストーラーを取得し、デジタル署名の署名者と証明書を確認してから、既存アプリへ再導入します。アプリ内更新は対象アプリの手順で行い、通常 NSIS を使うことを更新ログと取得したファイルで確認します。どちらも更新後の起動、バージョン、設定と利用者データの保持を[検証手順](verification.md)で確認します。
...
特に macOS のアプリ内更新は同じ bundle ID、証明書、秘密鍵を使う経路を前提にしているため、証明書の置換後も既存アプリが更新できると仮定しないでください。

新しい公開証明書を別経路で照合し、端末へ導入して信頼設定を確認します。
その確認とは別に、旧版からの移行を検証します。アプリ内更新で移行できない場合は、新しい署名を確認してから手動更新します。
