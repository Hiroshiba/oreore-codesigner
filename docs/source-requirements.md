...
すでにアプリ内更新を使うクライアントを配布している場合は、手動更新へ切り替える新版の公開前に、既存クライアントの実装と更新先 URL を確認し、新版の更新 metadata へ到達して自動取得・適用するかを判定します。
新版で手動更新を採用しても、旧版の自動取得・適用が止まるとは限りません。
自動適用する場合はその経路での実機更新を、しない場合は手動移行を検証し、設定と利用者データを保持する移行手順と利用者への案内を確認します。
初回の署名付き配布で更新元がない場合は、導入を確認したうえで「更新未確認」と記録し、次のバージョンで更新を検証します。

### 手動更新

macOS は対象 Release から新しい ZIP を取得し、展開したアプリの署名を確認してから、アプリを終了して既存のアプリを置き換えます。
Windows は対象 Release から新しい通常 NSIS インストーラーを取得し、署名を確認してから既存アプリへ再導入します。
