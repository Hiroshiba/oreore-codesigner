...
中央リポジトリの Actions から公開ワークフローを選び、`app_id`、`tag`、`replace_existing_assets` を入力します。
...
| 公開                           | 追加または置換された asset、残っている旧 asset、更新メタデータ | 一部公開済みの可能性があるため、実際の Release を確認してから判断する |
...
途中まで公開された場合は、新しいバージョンと Release で公開をやり直すか、管理者が実際の公開状態を調べて復旧方針を決めます。
...
自動ロールバックはしないため、ログと Release の状態を確認し、配布を続けてよいか判断します。
