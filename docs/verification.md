...
| 確認対象             | 合格条件                                                                                                                                                       |
| -------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 対象リポジトリと入力 | repository、tag、version で実行でき、App の Selected repositories の範囲だけへアクセスできる。version は必須で既定値がない                                     |
| ソース固定           | macOS と Windows が同じ source SHA を checkout し、公開前に tag が移動していれば停止する                                                                       |
| source契約           | 入力版と source の version がそれぞれ SemVer として検証され、exact pnpm、electron-builder 26.16.1、build script、単一 YAML、publish 禁止が検証される           |
| 配布版と channel     | source の version と異なる入力版でも実行でき、成果物と更新 metadata に入力版が反映される。`1.2.3` は `latest`、`0.1.1-edge.1` は `edge` になる                 |
...
| 公開先               | 既存かつ変更可能な Release だけを変更し、Release の本文、draft、prerelease などを編集しない                                                                    |
| 上書き公開           | 同名ファイルを置換し、別名の既存 asset は残す。配布ファイルと blockmap の後に更新 metadata を公開する                                                          |
| 公開失敗と再実行     | 途中状態を確認し、同じ実行の署名済み成果物で修復できる                                                                                                         |
| 後始末               | 一時的な P12、PFX、keychain、環境変数を削除し、処理と cleanup の両方の失敗を検出できる                                                                         |

## macOS 実機

更新の項目は、アプリ内更新を実装したアプリで確認します。連続する二つの配布 version を指定して用意します。
...
更新の項目は、アプリ内更新を実装したアプリで確認します。連続する二つの配布 version を指定して用意し、Smart App Control の状態を記録します。
...
実施日、実施者、OS と CPU、指定した version と channel、ソースの `package.json` の version、repository、tag、source SHA、Actions の実行 URL、公開証明書の fingerprint、取得したファイル名、結果とログの保存先を残します。
