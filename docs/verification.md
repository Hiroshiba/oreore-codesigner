...
4. 更新元がある場合は採用した方式で更新を確認します。アプリ内更新では旧版から新タグの Release URL へ到達し、差分更新と差分取得に失敗した場合の全量更新ができることを確認します。
...
| 更新の共通項目               | 同じ bundle ID、証明書、秘密鍵の次 version へ更新して起動でき、version、設定と利用者データの保持を確認できる             |
| 手動更新                     | アプリを終了し、新しい ZIP を展開して既存アプリを置き換えられる                                                          |
| アプリ内更新                 | 旧版が新タグの Release URL へ到達して ZIP を取得し、再起動時に適用できる                                                 |
| アプリ内更新の差分と全量取得 | 差分更新の成功をログで確認し、差分取得の失敗時にも全量取得で更新できる                                                   |
| 改変・別証明書               | 改変や異なる署名のアプリに対する OS の検証結果と、アプリ内更新を採用している場合は更新拒否と停止した検証段階を記録できる |
...
| 手動更新                     | 新しい通常 NSIS インストーラーで既存アプリへ再導入できる                                                                                     |
...
| 改変・別証明書               | 改変や期待しない署名のアプリ・インストーラーに対する OS の検証結果と、アプリ内更新を採用している場合は更新拒否と停止した検証段階を記録できる |
| 追加の端末                   | 公開証明書だけで端末を設定でき、秘密鍵の転送を要しない                                                                                       |
...
アプリ内更新を採用している場合は、新タグの Release URL への到達経路、差分取得と全量取得の結果も記録します。
